...
export function ProjectCard({ project }: { project: Project }) {
  ...
  // Stacking the visual above the copy made a featured card taller than
  // the viewport, so the whole project couldn't be taken in at once. The
  // title still spans the full width, but below it the visual and the
  // copy sit side by side from md up, which roughly halves the height.
  const bodyClass = hasVisual ? "md:grid md:grid-cols-5 md:items-stretch" : "";
  ...
  return (
    ...
    >
      ...
      <div className={`mt-4 md:mt-6 ${bodyClass}`}>
        ...
        <div
          className={`flex flex-col justify-center gap-3 p-5 md:gap-4 md:p-8 ${hasVisual ? "md:col-span-2" : ""}`}
        >
          <p
          ...
          {/* Label sits beside its value on phones and above it from md
              up — the stacked version costs height the card can't spare
              at 375px. */}
          <div
            className="mt-1 grid grid-cols-2 gap-4 border-t pt-3 md:mt-2 md:pt-4"
          ...
          >
            <div className="flex items-baseline gap-2 md:block">
              ...
              <p className="md:mt-1">{project.role}</p>
            </div>
            <div className="flex items-baseline gap-2 md:block">
              <span className={label} style={{ color: theme.textMuted }}>
                Tool
              </span>
              <p className="md:mt-1">{project.tool}</p>
            </div>
          </div>
        </div>
      </div>
