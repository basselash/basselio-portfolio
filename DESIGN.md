...
**Known accessibility gap, currently unresolved:** white text on `tertiary` (`#FD7250`) measures **2.73:1**, below WCAG AA (4.5:1) for text of any size. The source template ships this exact combination, and it was originally accepted here for fidelity while the coral carried display-scale text only. It now backs the hero's "Currently" card, which carries an 11px label and 16px body copy — so the original "display scale only" justification no longer holds, and this is the one real readability problem left in the system. The fix that costs nothing visually is switching that card's text to `on-surface`: black on the same unmodified coral measures **7.7:1**, clearing AAA while keeping the exact brand hex. Left as-is pending an explicit call, since it changes the card's look.
...
**Radius is the system's action/label signal, not decoration.** A full pill means "this is a static label you read" (skills, tech stack, section eyebrows); a 12px radius means "this is a control you can act on" (buttons, links out, inputs). When both shapes carried the same outline-and-muted-text treatment, the skill tags and the Email/LinkedIn/GitHub links became indistinguishable — a real reported confusion, not a hypothetical. Actions therefore also take darker text and a trailing ↗ glyph, and tags recede to `secondary`; shape, weight, and affordance all point the same direction rather than relying on radius alone.
...
**Résumé card (`CvDownloadCard`).** A `card-dark` variant that stands alone in a light section rather than living inside `section-dark` — same `on-surface`/`surface` pairing, same reasoning (reuse the one dark fill, don't invent a second). It's a server component that checks for the résumé file with `fs.existsSync` at render time and shows a working download link if present, or an honest "Coming soon" label if not — the same fallback discipline as `ProjectImage`/`ProjectVideo`, but resolved on the server since the file's presence is static and known at build time, rather than a client-side check that would flash the fallback state before resolving.
...
**Project video (`ProjectVideo`).** Two modes. The **inline preview** is muted, looped, `playsInline`, and paused via `IntersectionObserver` when scrolled out of view — it exists to show the work moving, not to be watched. Clicking it opens the **modal**, where the same recording plays full-size with native `controls` and sound, since a walkthrough is only useful if it can actually be watched, paused, and scrubbed. The preview is a real `<button>` with an accessible name so it's keyboard-reachable, and reveals a play affordance on hover/focus rather than looking decorative. The modal locks background scroll, closes on Escape or scrim click, moves focus to its close button, and hands focus back to the trigger on exit; the inline preview pauses while it's open so the same footage never plays twice. Falls back to a themed placeholder panel (using the project's own `surface`/`accent`) rather than a broken `<video>` or an invented screenshot when no real recording exists yet at the declared path.
...
- **Don't** give a static label the button shape, or an action the pill shape. Radius is how this system says "read this" vs "click this"; blurring the two is what made the skill tags and the social links read identically.
- **Do** let nav links glide to their section — `scroll-behavior: smooth` on `html`, wrapped in a `prefers-reduced-motion: no-preference` guard, with `scroll-mt-8` on every anchor target so a section never lands flush against the viewport edge.
